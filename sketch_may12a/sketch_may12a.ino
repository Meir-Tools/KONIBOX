...
void loop() {
  ...
  if (cardIndex == 0) {
    // כרטיס ראשון בזיכרון -> OFF (פקודת עצירה 0x16)
    Serial.println("Action: OFF (Card #0)");
    sendCommand(0x16, 0x00, 0x00);
  } 
  else if (cardIndex == 1) {
    // כרטיס שני בזיכרון -> ON (הפעלת שיר 1 - 0x03)
    Serial.println("Action: ON (Card #1)");
    sendCommand(0x03, 0x00, 0x01);
  } 
  else if (cardIndex > 1) {
    Serial.print("Card recognized at index: ");
    Serial.println(cardIndex);
    // כאן אפשר להוסיף פעולות לכרטיסים נוספים (3, 4 וכו')
  } 
  else {
...
void displayAllCards() {
  ...
  for (int i = 0; i < total; i++) {
    ...
    Serial.print(i == 0 ? "ID 0 (OFF): " : (i == 1 ? "ID 1 (ON): " : "ID "));
